...
import { labels, variants } from "npm:@catppuccin/palette";
...
const HEALTH_FILES = {
  repo: {
    coc:
      "https://raw.githubusercontent.com/catppuccin/.github/main/CODE_OF_CONDUCT.md",
    license:
  ...
  npm: {
    ...
    npmrc:
      "https://raw.githubusercontent.com/catppuccin/.github/main/health/npm/.npmrc",
  },
};
...
${
    chalk.hex(variants["macchiato"]["green"]["hex"])(
...
function fetch_file(out, url) {
  console.log(
    chalk.hex(variants["macchiato"]["blue"]["hex"])(
      "Downloading " + out + "...",
    ),
  );
  https.get(url, (res) => {
...
if (!(argv[0] in HEALTH_FILES)) {
  abort(
    "'" + argv[0] + "' is not a valid group of health files!\nTry '--help'",
  );
}
...
if (argv[1] == undefined) {
  for (let file in HEALTH_FILES[argv[0]]) {
    fetch_file(
      truncate_filename(HEALTH_FILES[argv[0]][file]),
      HEALTH_FILES[argv[0]][file],
    );
  }
} else {
  ...
  for (let file in files) {
    if (HEALTH_FILES[argv[0]][files[file]] != undefined) {
      fetch_file(
        truncate_filename(HEALTH_FILES[argv[0]][files[file]]),
        HEALTH_FILES[argv[0]][files[file]],
      );
    } else {
      console.error(
        chalk.hex(variants["macchiato"]["red"]["hex"])(
          "The health file '" +
            files[file] +
            "' is not in the group '" +
            argv[0] +
