...
import ColorContrastChecker from "npm:color-contrast-checker";
...
async function print_color(text, background, preferred_white, preffered_black) {
  await write(
...
await write(
  chalk.hex("#fff")("|Latte|\t\t|Frappe|\t|Macchiato|\t|Moccha|\n\n"),
);
for (let label in labels) {
  for (let palette in variants) {
    ...
    await write(symbol_space + good_contrast_ratio + "\t");
  }
  console.log("");
}
